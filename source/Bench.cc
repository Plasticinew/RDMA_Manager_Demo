...
void do_mem_cache_test(rdmanager::vQP* vqp, void* addr, int thread_id) {
    if(thread_id == 0)
        vqp->alloc_RPC(&remote_addr[0], &rkey[0], page_size*1024*1024*16);
    ...
    for(int j = 1; j < 65; j++){
        ...
        for(uint64_t i = 0; i < read_items; i++){
            vqp->read_main(addr, page_size, (void*)(remote_addr[0]+page_size*(rand_val()%(j*128*1024))), rkey[0]);
        }
...
int main(int argc, char* argv[]) {
    ...
    if(bench_type == "read") {
        ...
        for(int i = 0;i < thread_num; i++){
            std::thread* read_thread = new std::thread(&do_mem_cache_test, vqp_list[0], addr, i);
        }
    } else if(bench_type == "mr") {
        for(int i = 0;i < thread_num; i++){
            std::thread* read_thread = new std::thread(&do_mr_cache_test, vqp_list[0], addr, i);
        }
